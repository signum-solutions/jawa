...
class UserController extends BaseController
{
    ...
    public function getDashboard()
    {
      //print_r(session('timegenerated').'-'.time());die();
        ...
        //$homePageUrl = "https://app.powerbi.com/groups/b962df5a-da74-4a57-8cd5-9d751a9fd539/reports/91d763fe-f38b-43e3-8c65-ced73172d300/ReportSection8dfe45042d52b5d217ee";
        $homePageUrl = "https://app.powerbi.com/groups/b962df5a-da74-4a57-8cd5-9d751a9fd539/reports/91d763fe-f38b-43e3-8c65-ced73172d300/ReportSection8dfe45042d52b5d217ee?noSignUpCheck=1";
        list($groupId,$reportId,$sectionId) = splitUrlVals($homePageUrl);
    ...
    public function tokensessioncheck(){
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS,
                    "client_id=42283739-1b72-4a4c-af9e-6655ea649bd6&client_secret=kgR24]Ian9-M./7LoiODZ1FRb8rBv[=h&username=suntech.powerbi@kiteservices.com&password=analytics@123&resource=https://analysis.windows.net/powerbi/api&grant_type=password&scopes=openid");
    ...
    public function individualreport($id)
    {
        ...
        $categories = $admin_helper->getTabCategories($id);
        $data['categories'] = $categories;
        foreach($categories as $category){
            $subcategories[$category->category_id] = $admin_helper->getCategorySubCategories($category->category_id);
        }
        $data['subcategories'] = $subcategories;
        $a = reset($subcategories);
        $data['first_report'] = $a[0]->category_id;

	if(empty(session('accessToken')) || !empty(session('timegenerated')) || session('timegenerated') < time()-3600){
           $this->regenToken();
        }
        return view('contents.user.reportsnew')->with('data', $data);
    }
     ...
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     */
    public function getKpiBranchReports()
    {
        $admin_helper = new AdminHelper();
        ...
        return view('contents.user.kpi_reports2')->with('data', $data);
    }
    ...
    public function getEmbedToken($url,$reportUrl ='')
    {
      ...
      $url = "https://app.powerbi.com/groups/a51df78f-40d7-4c24-96d1-6f1af5054366/reports/517d2fc8-04a9-448c-9605-dcd6e7eebfcb/ReportSection";
        ...
        if($reportUrl !='') {
          list($groupId,$reportId,$sectionId) = splitUrlVals($reportUrl);  
        }
    ...
    public function getkpiCategoryData(Request $request)
    {
        $category_id = $request->input('category_id');

        //$user_helper = new UserHelper();
        //$iframes = $user_helper->getkpiCategoryIframestemp($category_id);      
        ...
        // } 
        if($category_id == 1){
            $reportUrl = "https://app.powerbi.com/groups/b962df5a-da74-4a57-8cd5-9d751a9fd539/reports/d13a3064-abe3-42bd-abb5-ae0276509062/ReportSection23f55b15e7941a576355";
        }else{
            $reportUrl = "https://app.powerbi.com/groups/b962df5a-da74-4a57-8cd5-9d751a9fd539/reports/1bde91f0-d98a-40a7-b18e-f0ab8a29d61e/ReportSectiond78ffd55bba25942a93d";
        }
    ...
    /**
     * Function to know the reports from the table
     *
     * @return mixed
     */
    public function getRep(Request $request){
        ...
        $data =  $admin_helper->getReportsTabList();
        if($catid == 0 && $subcatid == 0){
        dd($data);
        }
        elseif($catid != 0 && $subcatid == 0){
            $categories = $admin_helper->getTabCategories($catid);
            dd($categories);
        }elseif ($catid != 0 && $subcatid != 0) {
            $categories = $admin_helper->getTabCategories($catid);
            print_r($categories);
            $subcategories = $admin_helper->getCategorySubCategories($subcatid);
            print_r($subcategories);
        }elseif(FALSE){
            $user_helper = new \App\Models\UserHelper();
            $reports = $user_helper->getCategoryReports($category->category_id);
            print_r($reports);
        }
    ...
    /**
     * TO update the order added this functionality
     *
     * @return mixed
     */
    public function updateRepoOrder(Request $request){
        $catid = 0;
        if($request->has('catid')){
            $catid = $request->input('catid');
        }
        ...
        $admin_helper = new AdminHelper();
        if($catid != 0 && $orderby != 0){
            $admin_helper->updateOrder($catid,$orderby);
        }else{
            dd("input wrong");
        }
